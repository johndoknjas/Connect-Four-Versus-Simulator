...
//    void set_future_positions(const vector<unique_ptr<position>>& future_positionsP);
    ...
    bool can_create_threat_with_D(char piece) const; // Returns true if player with piece could possibly create a 4-in-a-row threat on
...
coordinate position::find_best_move_for_comp()
{
   ...
   /* auto rng = default_random_engine {};
    ...
    // Now try to play on D, unless this lets the opponent win by playing in the row barrier:
    ...
    for (const coordinate& current_move: possible_moves)
    {
        if (current_move.col == 3)
        {
            ...
            bool can_opponent_immediately_win_in_D = did_someone_win();
            ...
            if (!can_opponent_immediately_win_in_D)
            {
                return current_move;
            }
        }
    ...
    // Now should be only 1 possible move left: D. This loses immediately, but no other choice:
...
bool position::can_create_threat_with_D(char piece) const
{
    // See if piece can create a threat in/using column D, anywhere below the row_barrier square of D.
...
void position::smart_evaluation()
{
    ...
    if (row_barriers[3] != UNDEFINED && row_barriers[3] >= 1)
    {
        ...
        if (comp_has_the_chance && !can_create_threat_with_D('U'))
        {
            evaluation = INT_MAX;
        ...
        else if (!comp_has_the_chance && !can_create_threat_with_D('C'))
        {
            evaluation = INT_MIN;
