...
private:
    ...
    double find_revised_player_evaluation(const vector<coordinate_and_double>& info_for_player_amplifying_squares,
                                          const vector<vector<char>>& player_copy_board, const vector<vector<char>>& opponent_copy_board) const;
    ...
    void find_individual_player_evaluation(const vector<treasure_spot>& squares_amplifying_3,
                                          const vector<treasure_spot>& squares_amplifying_2, char piece,
                                          vector<vector<char>>& copy_board, vector<coordinate_and_double>& recorder) const;
                                                ...
                                                const vector<vector<char>>& player_copy_board, const vector<vector<char>>& opponent_copy_board) const
{
    ...
    {
        if (current.square.row + 1 <= max_row_index && opponent_copy_board[current.square.row + 1][current.square.col] == 'A')
        {
            temp_evaluation_as_double += current.value * 0.25;
        }

        else if (current.square.row + 2 <= max_row_index && player_copy_board[current.square.row + 2][current.square.col] == 'A')
        {
...
void position::smart_evaluation()
{
    ...
    evaluation = round(find_revised_player_evaluation(info_for_comp_valid_amplifying_squares, copy_board_for_comp, copy_board_for_user) -
                       find_revised_player_evaluation(info_for_user_valid_amplifying_squares, copy_board_for_user, copy_board_for_comp));
                                                ...
                                                vector<vector<char>>& copy_board, vector<coordinate_and_double>& recorder) const
{
    ...
    for (const treasure_spot& space: normal_squares_amplifying_2)
    {
        ...
        if (copy_board[current_square.row][current_square.col] == ' ')
        {
            bool is_next_square_empty = (is_in_bounds(next_square) && copy_board[next_square.row][next_square.col] == ' ' &&
                                         (row_barriers[next_square.col] == UNDEFINED || row_barriers[next_square.col] <= next_square.row));

            bool is_other_next_square_empty = (is_in_bounds(other_next_square) && copy_board[other_next_square.row][other_next_square.col] == ' ' &&
