...
//    void set_future_positions(const vector<unique_ptr<position>>& future_positionsP);
    ...
    bool can_create_threat(char piece) const; // Returns true if player with piece could possibly create a 4-in-a-row threat on
                                              // the bottom square of column D.
...
private:
    ...
    double find_revised_player_evaluation(const vector<coordinate_and_double>& info_for_player_amplifying_squares,
                                          ...
                                          const vector<vector<bool>>& squares_winning_for_opponent) const;
...
coordinate position::find_best_move_for_comp()
{
  ...
  /*  auto rng = default_random_engine {};
    ...
    // If control reached this point, the position has a situation with an amplifying square on D2.

    // First check if comp can play on D2 now:
    for (coordinate current_move: possible_moves)
    {
        if (current_move.col == 3 && current_move.row == 4)
        {
            return current_move;
        }
    }

    // Now "settle" for any move that doesn't move on D1:
    for (coordinate current_move: possible_moves)
    {
        if (!(current_move.col == 3 && current_move.row == 5))
        {
            return current_move;
        }
    }

    // Now should be only 1 possible move left: D1. This loses, but no other choice:
...
bool position::can_create_threat(char piece) const
{
    ...
    return (((*board)[5][2] != opponent_piece && (*board)[5][1] != opponent_piece && (*board)[5][0] != opponent_piece) ||
            ((*board)[5][4] != opponent_piece && (*board)[5][5] != opponent_piece && (*board)[5][6] != opponent_piece) ||
            ((*board)[4][2] != opponent_piece && (*board)[3][1] != opponent_piece && (*board)[2][0] != opponent_piece) ||
            ((*board)[4][4] != opponent_piece && (*board)[3][5] != opponent_piece && (*board)[2][6] != opponent_piece));
}
...
void position::smart_evaluation()
{
    ...
    if (row_barriers[3] == 4 && (*board)[5][3] == ' ')
    {
        // Figure out who has the chance to win using zugzwang on the square:

        bool comp_has_the_chance = false;

        // Includes all empty squares (off col D) and the bottom square of col D.
        // Basically all squares that can be played before someone can win with the amplifying square second lowest in D.
        int number_of_empty_squares = 42 - number_of_pieces - 5;

        if (number_of_empty_squares % 2 == 0)
        {
            comp_has_the_chance = is_comp_turn;
        }

        else // odd number of playable squares before armageddon
        {
            comp_has_the_chance = !is_comp_turn;
        }

        // Now ensure the opponent of the player who has the chance cannot create a 4-in-a-row threat on
        // the square under D's row barrier amplifying square:

        if (comp_has_the_chance && !can_create_threat('U'))
        {
            evaluation = INT_MAX;
        ...
        else if (!comp_has_the_chance && !can_create_threat('C'))
        {
            evaluation = INT_MIN;
    ...
    evaluation = round(find_revised_player_evaluation(info_for_comp_valid_amplifying_squares,
                                                      board_for_squares_winning_for_comp, board_for_squares_winning_for_user) -
                       find_revised_player_evaluation(info_for_user_valid_amplifying_squares,
                                                      board_for_squares_winning_for_user, board_for_squares_winning_for_comp));
